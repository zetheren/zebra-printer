...
2. Writes the leaf certificate and unencrypted private key into separate in-memory PEM payloads.
3. Deletes old `HTTPS_CERT.NRD`, `HTTPS_KEY.NRD`, and `HTTPS_CA.NRD` objects.
4. Uploads the private key as `E:HTTPS_KEY.NRD` and the leaf certificate as `E:HTTPS_CERT.NRD` with Zebra's `~DY` binary command.
5. Confirms that both objects exist before requesting a printer reset.
...
- `! U1 do "file.dir" "E:"`
- `~DYE:HTTPS_KEY.NRD,B,NRD,<size>,,<private-key-content>`
- `~DYE:HTTPS_CERT.NRD,B,NRD,<size>,,<certificate-content>`
- `! U1 do "device.reset" ""`
